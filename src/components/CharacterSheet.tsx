...
const characterData = {
    ...
    baseStats: [
      ...
      { id: "chest", label: "Core", value: "Programmer", barPercentage: "70%", targetPosition: { top: "70%", left: "50%" } }, 
      { id: "legs", label: "Foundation", value: "Mathematician", barPercentage: "80%", targetPosition: { top: "70%", left: "70%" } }, 
      { id: "projects", label: "Stamina", value: "Growing portfolio", barPercentage: "50%", targetPosition: { top: "60%", left: "70%" } },
    ],
    xp: 7500,
    xpToNextLevel: 10000,
};
    ...
    >
      <div className="flex flex-col md:flex-row w-full">
        ...
        <div className="w-full md:w-1/3 p-4 flex items-center justify-center">
          <div className="flex flex-col mt-8 gap-8 w-full max-w-xs sm:max-w-sm">
            ...
            <div className="bg-background/60 backdrop-blur-sm p-4 rounded-lg text-center">
                <p className="text-xl sm:text-2xl font-bold text-foreground drop-shadow-sm mb-4">
                    Toby Chen
                </p>
                <div className="mt-1 text-xs sm:text-sm font-semibold">
                  <div className="gap-2 flex flex-col sm:flex-row justify-center">
                      <span className="bg-primary/20 text-foreground px-3 py-1 border-2 border-foreground rounded-xl flex justify-center">
                          Lv. {characterData.level} Arch Linux Loyalist
                      </span>
                      <span className="bg-green-500/20 border-2 border-green-800/50 text-green-800 px-3 py-1 rounded-xl flex justify-center">
                          {characterData.alignment}
                      </span>
                  </div>
                  <p className="bg-primary/20 border-2 border-primary/50 text-primary px-3 py-1 rounded-xl mt-2">
        ...
        <div className="w-full md:w-2/3 p-6 flex flex-col">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-4">
            ...
            <div className="flex flex-wrap justify-center sm:justify-end gap-2 text-xl sm:text-2xl">
              {characterData.types.map(type =>
                <div 
                  key={type.name} 
                  className="px-3 py-2 sm:px-4 bg-primary/90 rounded-md text-background flex items-center justify-center" 
                  title={type.name}
                >
                  {type.icon}
                </div>
          ...
          <p className="text-foreground/90 mb-6 text-center sm:text-left text-sm sm:text-base">
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 mb-4">
            <button 
              ...
              className={`py-2 px-4 text-sm sm:text-base font-semibold rounded-md transition-all duration-200 ease-in-out ${view === 'base' ? 'bg-background text-foreground shadow-md' : 'text-primary hover:bg-primary/90 hover:text-background bg-background/50'}`}
            >
              Base Stats
            </button>
            <button 
              onClick={() => setView('attributes')} 
              className={`py-2 px-4 text-sm sm:text-base font-semibold rounded-md transition-all duration-200 ease-in-out ${view === 'attributes' ? 'bg-background text-foreground shadow-md' : 'text-primary hover:bg-primary/90 hover:text-background bg-background/50'}`}
            >
              Attributes
            </button>
            <button 
              onClick={() => setView('radar')} 
              className={`py-2 px-4 text-sm sm:text-base font-semibold rounded-md transition-all duration-200 ease-in-out ${view === 'radar' ? 'bg-background text-foreground shadow-md' : 'text-primary hover:bg-primary/90 hover:text-background bg-background/50'}`}
            >
              Skill Radar
            </button>
          </div>

          <div className="min-h-[300px] sm:min-h-[350px] flex-grow flex flex-col justify-center bg-background/40 rounded-lg p-4 sm:p-8">
