...
const TABS: { key: CharacterSheetView; label: string }[] = [
    { key: 'special-items', label: 'Special Items' },
    { key: 'rare-encounters', label: 'Rare Encounters' },
...
const rarityColors: Record<Rarity, { border: string, bg: string, text: string }> = { "Common": { border: "border-gray-400", bg: "bg-gray-500/20", text: "text-gray-300" }, "Uncommon": { border: "border-green-500", bg: "bg-green-500/20", text: "text-green-400" }, "Rare": { border: "border-blue-500", bg: "bg-blue-500/20", text: "text-blue-400" }, "Epic": { border: "border-purple-500", bg: "bg-purple-500/20", text: "text-purple-400" }, "Legendary": { border: "border-amber-500", bg: "bg-amber-500/20", text: "text-amber-400" }, };
const difficultyColors: Record<Difficulty, { border: string, bg: string, text: string }> = { "Trivial": { border: "border-gray-400", bg: "bg-gray-500/20", text: "text-gray-300" }, "Easy": { border: "border-green-500", bg: "bg-green-500/20", text: "text-green-400" }, "Normal": { border: "border-sky-500", bg: "bg-sky-500/20", text: "text-sky-400" }, "Hard": { border: "border-orange-500", bg: "bg-orange-500/20", text: "text-orange-400" }, "Heroic": { border: "border-red-500", bg: "bg-red-500/20", text: "text-red-400" }, "Deadly": { border: "border-purple-600", bg: "bg-purple-600/20", text: "text-purple-400" }, };
const characterData = { name: "Toby Chen", title: "Mathematician & Programmer", level: 21, alignment: "Lawful Good", types: [ { icon: <FaReact />, name: "React" }, { icon: <FaPython />, name: "Python" }, { icon: <SiTypescript />, name: "TypeScript" }, { icon: <SiCplusplus />, name: "C++" } ], description: "3rd year Pure Mathematics student at Queen Mary University of London. Growing programmer, math practitioner. Aspiring to obtain job that uses mathematical and programming skills.", baseStats: [ { id: "head", label: "Intelligence", value: "Pure Mathematics Fanatic", barPercentage: "90%" }, { id: "arm-right", label: "Power", value: "Athlete", barPercentage: "60%" }, { id: "chest", label: "Core", value: "Programmer", barPercentage: "70%" }, { id: "legs", label: "Foundation", value: "Mathematician", barPercentage: "80%" }, { id: "projects", label: "Stamina", value: "Growing portfolio", barPercentage: "50%" }, ], xp: 2316, xpToNextLevel: 10000, };
...
const FilterControls: FC<FilterControlsProps> = ({ filters, currentFilter, onFilterChange }) => (
    <div className="flex items-center flex-wrap gap-2 mb-4"> {/* // --- MODIFIED --- Added flex-wrap for small screens */}
        {filters.map(filter => (
...
const LogbookView: FC<LogbookViewProps> = ({ title, items, filters, currentFilter, onFilterChange, renderItem, emptyText }) => (
    <motion.div key={title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
        ...
        {/* // --- MODIFIED --- Adjusted grid columns for narrower container on mobile */}
...
const CharacterProfile: FC = () => (
    // --- MODIFIED --- Set responsive widths and prevent shrinking
...
// --- SECTION 4: Main Component (Refactored) ---
...
function CharacterSheet() {
    ...
    return (
        ...
        >
            ...
            {/* // --- MODIFIED --- Changed to flex-row permanently */}
            <div className="flex flex-row w-full">
                <CharacterProfile />

                {/* // --- MODIFIED --- Set responsive widths, allow growing, and fix overflow potential */}
                <div className="w-3/5 lg:w-2/3 p-2 sm:p-6 pt-0 md:pt-4 flex flex-col flex-grow min-w-0">
                    <div role="tablist" className="flex flex-wrap gap-2 mb-4">
                        {TABS.map(tab => (
                            <button key={tab.key} onClick={() => handleViewChange(tab.key)} role="tab" aria-selected={view === tab.key} className={`py-1 px-2 sm:py-2 sm:px-4 text-xs sm:text-base font-semibold rounded-md transition-all duration-200 ease-in-out ${view === tab.key ? 'bg-background text-foreground shadow-md' : 'text-primary hover:bg-primary/90 hover:text-background bg-background/50'}`}>
                                {tab.label}
                            </button>
                        ))}
                    </div>
