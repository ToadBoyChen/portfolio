...
interface LogbookViewProps {
    ...
    currentFilter: FilterStatus;
    onFilterChange: (filter: FilterStatus) => void;
    renderItem: (item: JourneyStep) => ReactNode;
    emptyText: string;
}
    ...
    <div className="w-full md:w-1/3 p-4 flex items-center justify-center">
        <div className="flex flex-col mt-8 gap-8 w-72 mx-auto">
            ...
            <div className="bg-background/60 backdrop-blur-sm p-4 rounded-lg text-center">
                <p className="text-xl sm:text-2xl font-bold text-foreground drop-shadow-sm mb-4">{characterData.name}</p>
...
function CharacterSheet() {
    ...
    const filteredQuests = useMemo(() => {
        switch (filterStatus) {
            case 'achieved':
                return journeySteps.filter(j => j.progress === 100);
            case 'in-progress':
                return journeySteps.filter(j => j.progress > 0 && j.progress < 100);
            case 'pending':
                return journeySteps.filter(j => j.progress === 0);
            case 'all':
            default:
                return journeySteps;
        }
        ...
        >
            <div className="flex flex-col md:flex-row w-full">
                ...
                <div className="w-full md:w-2/3 p-6 flex flex-col">
                    ...
                    <div role="tablist" className="flex flex-wrap gap-2 sm:gap-4 mb-4">
                        {TABS.map(tab => (
                            <button key={tab.key} onClick={() => handleViewChange(tab.key)} role="tab" aria-selected={view === tab.key} className={`py-2 px-4 text-sm sm:text-base font-semibold rounded-md transition-all duration-200 ease-in-out ${view === tab.key ? 'bg-background text-foreground shadow-md' : 'text-primary hover:bg-primary/90 hover:text-background bg-background/50'}`}>
                                {tab.label}
                            </button>
                        ))}
                    </div>
                        ...
                        <AnimatePresence mode="wait">
                            ...
                            {view === 'special-items' && (
                                <LogbookView
                                    ...
                                    renderItem={(journey) => (
                                        <Tooltip key={journey.title} content={ <> <p className={`font-bold text-sm ${rarityColors[journey.specialItemRarity].text}`}>{journey.specialItem}</p> <p className="font-semibold">{journey.specialItemRarity}</p> <hr className="my-1 border-foreground/20" /> <p><span className="font-semibold">Source:</span> {journey.title}</p> </> }>
                                            <div tabIndex={0} className={`aspect-square p-2 rounded-lg border-2 transition-all duration-300 ${rarityColors[journey.specialItemRarity].border} ${rarityColors[journey.specialItemRarity].bg} hover:scale-105 hover:shadow-lg focus:scale-105 focus:shadow-lg outline-none cursor-pointer`}>
                            ...
                            {view === 'rare-encounters' && (
                                <LogbookView
                                    title="Encounter Logbook"
                                    items={filteredQuests}
                                    filters={encounterFilters}
                                    currentFilter={filterStatus}
                                    onFilterChange={setFilterStatus}
                                    emptyText="No encounters match this filter."
                                    renderItem={(journey) => (
                                        <Tooltip key={journey.title} content={ <> <p className={`font-bold text-sm ${difficultyColors[journey.difficulty].text}`}>{journey.title}</p> <p className="font-semibold">{journey.difficulty}</p> <hr className="my-1 border-foreground/20" /> <p><span className="font-semibold">Location:</span> {journey.location}</p> </> }>
                                            <div tabIndex={0} className={`aspect-square p-2 rounded-lg border-2 transition-all duration-300 ${difficultyColors[journey.difficulty].border} ${difficultyColors[journey.difficulty].bg} hover:scale-105 hover:shadow-lg focus:scale-105 focus:shadow-lg outline-none cursor-pointer`}>
